...
def search_from_commits(commit_list, author_name):
    ...
    for commit in commit_list:
        ...
        if (
            ...
            and prev_saturday <= commit.committed_datetime
            and commit.message[:2] == "No"
        ):
            # print (commit.author, commit.message)
            logger.info(
                "Commit Info: [{}] author: {} - {}".format(
                    commit.committed_datetime, commit.author, commit.summary
                )
            )
...
def slice_commit(my_commits):
    ...
    for commit_str in my_commits:
        idx = min(
            commit_str.find("_") % 10000,
            commit_str.find(" ") % 10000,
            commit_str.find("の") % 10000,
        )
...
def execute(git_repo_path, author):
    ...
    for repo_data in git_repo_path:
        ...
        repo_type = (
            RepoType.RECOVERY
            if repo_data["type"] == "Recovery"
            else RepoType.CREATE_TEXT
        )
